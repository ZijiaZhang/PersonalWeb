...
export interface ExperienceProps {
    ...
    icon: "fa-chalkboard-teacher"|"fa-vr-cardboard"|"fa-laptop"|"fa-search";
    title: string;
    location: string;
    body: string;
    date: string;
...
export class Experience extends React.Component<ExperienceProps & {left: boolean}, {}>{
    render(){
        console.log(this.props.left);
        return (<div className="cd-timeline-block">
            <div className={"cd-timeline-img " + this.props.color} data-aos="zoom-in">
                <i className={"fas " + this.props.icon}/>
            </div>
            <div className="cd-timeline-content" data-aos={mobile() ? "": (this.props.left?'fade-left':'fade-right')}>
                <h2>{this.props.title}</h2>
                <h5><i className="fa fa-map-marker"/>{this.props.location}</h5>
                <p>{this.props.body}</p>
                <span className="cd-date" data-aos="zoom-in">{this.props.date}</span>
                {this.props.link? <a href={this.props.link} className="cd-read-more"> Link to Work</a> : ""}
            </div>
        </div>);
    }
}